...
object CreateTable2 {
  def main(args: Array[String]): Unit = {
    ...
    val dataStream1: DataStream[(String, Long, Double)] = env.readTextFile(getClass.getResource("/sensor.txt").getPath)
      .map(data => {
        val dataArrs: Array[String] = data.split(",")
        (dataArrs(0).trim, dataArrs(1).trim.toLong, dataArrs(2).trim.toDouble)
      })

    val dataStream2: DataStream[SensorReading] = env.readTextFile(getClass.getResource("/sensor.txt").getPath)
      .map(data => {
        val dataArrs: Array[String] = data.split(",")
