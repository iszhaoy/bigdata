...
object TestWindow {
  def main(args: Array[String]): Unit = {
    ...
    val minTempPerWindowStream: DataStream[(String, Double)] = dataStream
      ...
      //.process(new ProcessWindowFunction[(String,Double),String,String,TimeWindow] {
      //  override def process(key: String, context: Context, elements: Iterable[(String, Double)], out: Collector[String]): Unit = {
      //    for (elem <- elements) {
      //      // 统计出现的次数等
      //    }
      //  }
